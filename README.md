...
| `SPOND_EXCLUDE_GROUP_ID` | Valgfritt: Gruppe-ID eller subgroup-ID. Medlemmer ekskluderes fra svømmere og slettes ved sync. Styret er typisk en subgroup. |
...
5. Valgfritt: Sett `SPOND_EXCLUDE_GROUP_ID` til gruppens eller undergruppens ID (f.eks. Styret). Disse importeres ikke og slettes automatisk ved sync hvis de allerede er i databasen. Styret er typisk en subgroup – subgroup-IDene finnes i Spond API-responsen under `subGroups` i hver gruppe.
